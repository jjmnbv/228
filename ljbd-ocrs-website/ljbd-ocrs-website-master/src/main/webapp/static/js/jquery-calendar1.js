...
var popUpCal = {
	...
	showCalendar: function() {
		...
		var timeSelect = '';
		if (this.timeSeparators && this.timeSeparators.length > 1) {
			timeSelect += '<div id="calendar_time"><div class="s-style">'+this.selectedHour+'<select id="calendar_hour">';
			...
			if (this.timeSeparators.length == 4) {
				...
				for (var a = 1; a < 12; a++) {if(a<10){
					if (this.selectedHour == a || (a == 12 && this.selectedHour == 0) || (a + 12 == this.selectedHour)) timeSelect += '<option value="' + a + '" selected="selected">' +'0'+ a + '</option>';
					else timeSelect += '<option value="' + a + '">' +'0'+a + '</option>';}
					else{if (this.selectedHour == a || (a == 12 && this.selectedHour == 0) || (a + 12 == this.selectedHour)) timeSelect += '<option value="' + a + '" selected="selected">' + a + '</option>';
			...
			timeSelect += '</select> ' +'</div>' +'<div class="s-style">'+ this.selectedMinute+' <select id="calendar_minute">';
			for (var a = 0; a < 59; a=a+5) {
			...
			timeSelect +='</select>'+'</div>';
				if(this.selectedHour < 12)   var  ampm="AM";
				else  ampm="PM";
			if (this.timeSeparators.length == 4) {
			
				
				timeSelect +='<div class="s-style am">'+ampm+' <select id="calendar_ampm">';
		...
		if (!this.changeMonth) {
			...
			html += '<div class="s-style">'+this.monthNames[this.selectedMonth]+'<select id="calendar_newMonth">';
		...
		if (!this.changeYear) {
			...
			html += '<div class="s-style">'+this.selectedYear+'<select id="calendar_newYear">';
	...
	init: function() {
		...
		this.lastInput = null;
		this.disabledInputs = [];
		$('.searchbox').append('<div id="calendar_div"></div>');
	...
	setPos: function(targetObj, moveObj) {
		...
		moveObj.css('position', 'absolute').css('left', coords[0]-377+'px').
			css('top', (coords[1] + targetObj.offsetHeight)-153 + 'px');
	},
...
$(function(){
		   $(".c1").click(function(){
						...
						$("#calendar_div").css("left","40px");					  })
		   
		    $(".c2").click(function(){
											  
						$("#calendar_div").css("left","299px");					  })
